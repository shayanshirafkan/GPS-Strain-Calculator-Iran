...
Q(2*i-1,2*i)=gps(i,5)*gps(i,6) * 10^(-6)*gps(i,7) ;
Q(2*i,2*i-1)=gps(i,5)*gps(i,6) * 10^(-6)*gps(i,7) ;
...
[ddy1,ddy2]=distance(r(i,4),r(i,5),gps(j,2),gps(j,1));

ddy1=deg2km(ddy1)*1000;

ddx=ddy1*sind(ddy2);
ddy=ddy1*cosd(ddy2);
A(2*j-1,:)=[1  0 ddy ddx   ddy  0];
A(2*j,:)=[0   1 -ddx    0     ddx    ddy];
end
q=zeros(798,798);
...
for k=1:399;
n1=[];
n1=deg2km(distance(r(i,4),r(i,5),gps(k,2),gps(k,1)))*1000;
h(k,1)=n1;
n1=exp(-(n1/(10^3))^2/25000);
q(2*k-1,2*k-1)=n1;
q(2*k,2*k)=n1;
end
   hh=[];
...
tt=str(:,4)*10^9;;
scatterm(xx,yy,20,tt,'filled')
xlabel('X');ylabel('Y');C=colorbar;C.Label.String='nono strain';
title('I_2')
axis equal
